...
class CodeLinesSplitter(TextSplitter):
    """
    Class that splits code files into groups of code lines.
    Currently set to 100 lines of code
    """

    def __init__(self, max_lines_per_section: int = 100):
    ...
    def split_pages(self, pages: List[Page]) -> Generator[SplitPage, None, None]:
        ...
        if all_text.count("\n") <= self.max_lines_per_section:
            yield SplitPage(page_num=0, text=all_text, isSplitted=False)
            return
        
        # its too big, so we need to split it
        lines = all_text.split("\n")
        for i in range(0, len(lines), self.max_lines_per_section):
            yield SplitPage(page_num=i // self.max_lines_per_section, text="\n".join(lines[i : i + self.max_lines_per_section]), isSplitted=True)
        return
